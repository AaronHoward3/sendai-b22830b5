...
import { Eye, Copy, Trash2, X, ChevronDown, Plus, Download, Code, Code2, FileText } from 'lucide-react';
...
const MyEmails: React.FC = () => {
  ...
  if (!user) {
    return <div className="min-h-screen bg-background text-foreground">
      <Navigation />
      <div className="pt-16 container mx-auto px-4 py-10 space-y-4"><Card>
        <CardHeader>
          <CardTitle>Sign in required</CardTitle>
          <CardDescription>Sign in on the Dashboard to view your saved emails.</CardDescription>
        </CardHeader>
        <CardContent>
          <GradientButton onClick={() => navigate('/dashboard')} className="!bg-primary !text-primary-foreground">Go to Dashboard</GradientButton>
        </CardContent>
    </Card></div></div>
  }
  return <div className="min-h-screen bg-background text-foreground">
      <Navigation />
      <div className="container mx-auto pb-8 space-y-6 max-w-4xl pt-24 px-0">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
          ...
          <GradientButton onClick={handleCreateNew} variant="solid"
          ...
          ><Plus className="h-4 w-4" />Create New</GradientButton>
        </div>
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center justify-between flex-wrap gap-2">Saved Email Templates<Badge variant="secondary">{rows.length} Templates</Badge></CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            {rows.length === 0 ? (
              <div className="text-center py-8">
                ...
                <div key={row.id} className="flex flex-col sm:flex-row sm:justify-between gap-4 p-4 border rounded-2xl bg-muted/40">
                  <div className="space-y-1 min-w-0">
                    <h3 className="font-semibold text-base text-foreground truncate">{row.subject || '(No Subject)'}</h3>
                    <p className="text-sm text-muted-foreground truncate">{row.brand_domain || 'Unknown domain'} • {(row.style_meta as Record<string, string>)?.emailType || 'Unknown'} • {formatDate(row.created_at)}</p>
                  </div>
                  <div className="flex flex-wrap gap-2 sm:justify-end relative">
                    <TooltipProvider><Tooltip delayDuration={0}>
                      <TooltipTrigger asChild><Button
                          variant="outline" size="sm" onClick={() => setExpandedEmailId(row.id)}
                          className="!bg-background !text-foreground !border !border-border hover:!bg-muted"
                      ><Eye className="h-4 w-4" /></Button></TooltipTrigger>
                      <TooltipContent>View</TooltipContent>
                    </Tooltip></TooltipProvider>
                    <div className="relative inline-block" onMouseEnter={() => setCopyDropdownOpenId(row.id)} onMouseLeave={() => setCopyDropdownOpenId(null)}>
                      <TooltipProvider><Tooltip delayDuration={0}>
                        <TooltipTrigger asChild><Button variant="outline" size="sm" className="!bg-background !text-foreground !border !border-border hover:!bg-muted gap-1">
                            <Copy className="h-4 w-4" /><ChevronDown className="h-3 w-3" />
                      ...
                      {copyDropdownOpenId === row.id && (
                        <div className="absolute top-[100%] right-0 bg-popover border rounded-md shadow-md z-50 overflow-hidden">
                          <button onClick={() => handleCopy(row.mjml, 'MJML')} className="flex w-full text-left px-4 py-2 text-sm hover:bg-accent items-center gap-1">
                            <FileText className="h-4 w-4" />MJML</button>
                          <button onClick={() => handleCopy(row.html, 'HTML')} className="flex w-full text-left px-4 py-2 text-sm hover:bg-accent items-center gap-1">
                            <Code2 className="h-4 w-4" />HTML</button>
                        </div>
                      )}
                    </div>
                    <div className="relative inline-block" onMouseEnter={() => setDownloadDropdownOpenId(row.id)} onMouseLeave={() => setDownloadDropdownOpenId(null)}>
                       <TooltipProvider><Tooltip delayDuration={0}>
                          <TooltipTrigger asChild><Button variant="outline" size="sm" className="!bg-background !text-foreground !border !border-border hover:!bg-muted gap-1">
                      ...
                      {downloadDropdownOpenId === row.id && <div className="absolute top-[100%] right-0 bg-popover border rounded-md shadow-md z-50 overflow-hidden">
                            <button onClick={() => handleDownload(row.mjml, `${row.subject || 'email'}.mjml`, 'MJML')}
                             className="flex w-full text-left px-4 py-2 text-sm hover:bg-accent items-center gap-1">
                              <FileText className="h-4 w-4" />MJML</button>
                    ...
                    <TooltipProvider><Tooltip delayDuration={0}>
                      <TooltipTrigger asChild><Button
                          variant="outline" size="sm" onClick={() => handleDelete(row.id)}
                          className="!bg-background !text-foreground !border !border-border hover:!bg-muted"
                      ><Trash2 className="h-4 w-4" /></Button></TooltipTrigger>
      ...
      {expandedEmailId && currentEmail && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4"><div className="relative">
            <div className="absolute -top-4 -right-4 z-10">
              <button onClick={() => setExpandedEmailId(null)}
                  ...
                  <div className="p-4 border-b border-border flex justify-between items-center">
                    ...
                    <div className="flex gap-2">
                      <GradientButton size="sm" variant="white-outline"
                        ...
                        <Copy className="w-4 h-4 mr-1" /> Copy MJML
                      </GradientButton>
                      <GradientButton size="sm" variant="white-outline"
                        onClick={() => handleDownload(currentEmail.mjml, `${currentEmail.subject || 'email'}.mjml`, 'MJML')}
                        className="hover:scale-105 !bg-background !text-foreground !border !border-border hover:!bg-muted">
                        <Download className="w-4 h-4 mr-1" /> Download MJML
                      </GradientButton>
                    </div>
                  </div>
